...
public class RequestPerformer extends Behaviour{
  ...
  public RequestPerformer(AgenteComprador a) {
    bbAgent = a;
        ...
        reply = myAgent.receive(mt);
            if (reply != null) {
                if (reply.getPerformative() == ACLMessage.INFORM) {
                    System.out.println(bookTitle+" compra con exito del agente  "+reply.getSender().getName());
                    System.out.println("Precio = "+bestPrice);
                    JOptionPane.showConfirmDialog(null,"Se compro el libro "+ bookTitle+" exitosamente con el agente \n "+reply.getSender().getName()+" por un precio de $"+bestPrice, "Compra Exitosa", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE);
                    myAgent.doDelete();
                }
            else {
